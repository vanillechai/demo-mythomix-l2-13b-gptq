...
# Banana.dev MythoMix-L2-13B-GPTQ starter template

This is a MythoMix-L2-13B-GPTQ starter template from [Banana.dev](https://www.banana.dev) that allows on-demand serverless GPU inference.
...
1. [Fork this](https://github.com/bananaml/demo-mythomix-l2-13b-gptq/fork) repository to your own Github account.
...
Build the model as a Docker image. You can change the `mythomix-l2-13b-gptq` part to anything.

```sh
docker build -t mythomix-l2-13b-gptq .
```

Run the Potassium server

```sh
docker run --publish 8000:8000 -it mythomix-l2-13b-gptq
```
